...
const fetchData = async () => {
    ...
    for (let page = 1; page <= MAX_REQUESTS; page++) {
        promises.push(
            fetch(`https://catalog.api.2gis.com/3.0/items?q=Green%20House&location=${center[0]}%2C${center[1]}&key=b5ad917c-78ca-4c9d-9265-44d22d87ffa7&page_size=10&fields=items.point,items.external_content,items.full_address_name,items.org&radius=1000&page=${page}`)
                .then(response => {
                    if (!response.ok) {
    ...
    const mergedItems = responses
        .filter(response => response && response.result && response.result.items) 
    ...
    coords.forEach(coord => {
        new mapgl.Marker(mapRef.current!, {
            coordinates: coord,
            icon: '/src/assets/icons/green-icon.svg'
        });
    });
} catch (error) {
